...
public class Game {
  ...
  public void tick(GameMap map, Inputs inputs, Outputs outputs) {
    ...
    int nextX = ((vlak.getCoords().x + currentDirection.x + map.getWidth()) % map.getWidth());
    int nextY = ((vlak.getCoords().y + currentDirection.y + map.getHeight()) % map.getHeight());
    vlak.move(nextX, nextY);
  ...
  private void addPassenger() {
    map.addObject(randy.nextInt(map.getWidth()), randy.nextInt(map.getHeight()), Field.GROUND, new GameObject(Sprite.PASSENGER1));
  }
